...
class CatalogSessionActivity implements SessionActivity {
  ...
  Future<void> end() async {
    ...
    // Read from the catalog, so it goes when the catalog does (UC-20), or the
    // next sign-in would start out showing the library the session that just
    // ended read.
    ...
    _ref.invalidate(bookmarkCollectionsProvider);
    _ref.invalidate(bookmarkCollectionFilterProvider);
