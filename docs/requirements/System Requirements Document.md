...
| FR-PL-07 | The system shall present a full-window player for what is playing: the album's own picture, the track's name, the artist whose record it is, the album it is from, and a moving indication that sound is playing which runs while playback runs and settles when it stops. It shall show the album's own picture in the playback bar as well, and where a file carries no picture it shall show a placeholder in the sleeve's own shape rather than an empty frame. |
